...
func (s *service) UpdateField(ctx context.Context, requestTurno2 RequestTurno2, id int) (Turno, error) {
	...
	turnoFromDB, err := s.repository.GetByID(ctx, id)
	if err != nil {
		log.Println("Error en servicio de turno:", err.Error())
		return Turno{}, errors.New("error en servicio. Metodo GetByID desde updateFild")
	}

	turno := requestToTurno2(requestTurno2, turnoFromDB)
...
func requestToTurno2(userReq RequestTurno2, turno Turno) Turno {
	...
	if userReq.Paciente != nil {
		turno.Paciente = *userReq.Paciente
	}

	if userReq.Odontologo != nil {
		turno.Odontologo = *userReq.Odontologo
	}
